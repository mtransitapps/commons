...
STORE_PRODUCTION_FILE="${CONFIG_DIR}/store/production";
STORE_BETA_FILE="${CONFIG_DIR}/store/beta";
STORE_BETA_PRIVATE_FILE="${CONFIG_DIR}/store/beta-private";
if [[ -f "$STORE_PRODUCTION_FILE" ]]; then
...
[Join the Beta](https://play.google.com/apps/testing/$PKG)
EOL
)
elif [[ -f "$STORE_BETA_PRIVATE_FILE" ]]; then
  STORE_SECTION=$(cat <<EOL

...
if [[ ${IS_CI} = false ]]; then
	...
	../gradlew ${SETTINGS_FILE_ARGS} :${DIRECTORY}:bundleRelease; # no ${GRADLE_ARGS} for release
	RESULT=$?;
	checkResult ${RESULT};
	...
	../gradlew ${SETTINGS_FILE_ARGS} :${DIRECTORY}:assembleRelease -PuseGooglePlayUploadKeysProperties=false; # no ${GRADLE_ARGS} for release
	RESULT=$?;
	checkResult ${RESULT};
